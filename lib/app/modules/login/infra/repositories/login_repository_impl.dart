...
class LoginRepositoryImpl implements LoginRepository {
  ...
  Future<Either<Failure, LoggedUserInfo>> loginEmail({String email, String password}) async {
    try {
      ...
      return Left(ErrorLoginEmail(message: "Error login with Email: ${e.toString()}"));
    }
  }
  ...
  @override
  Future<Either<Failure, LoggedUserInfo>> verifyPhoneCode({String verificationId, String code}) async {
    try {
      var user = await dataSource.validateCode(verificationId: verificationId, code: code);
      return Right(user);
    } catch (e) {
      return Left(ErrorLoginPhone(message: "Error login with Phone"));
    }
